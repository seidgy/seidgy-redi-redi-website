...
export interface NuxtWebsiteProps {
  /**
  ...
  readonly alternateHostedZones?: aws_route53.IHostedZone[]
  ...
  /**
   * The cache policy to be used on the default behavior.
   *
   * @default - CachePolicy.CACHING_OPTIMIZED
   */
  readonly cachePolicy?: aws_cloudfront.CachePolicy | aws_cloudfront.ICachePolicy
   ...
   */
  readonly securityHeadersBehavior?: aws_cloudfront.ResponseSecurityHeadersBehavior | aws_cloudfront.IResponseHeadersPolicy

  /**
   * An existent response header policy id to be used instead of creating a new one.
   *
   * @default - none
   */
...
export class NuxtWebsite extends Construct {
  public readonly distribution: aws_cloudfront.Distribution
  public readonly bucket: aws_s3.Bucket
  public readonly origin: aws_cloudfront_origins.S3Origin

  constructor(scope: Construct, id: string, props: NuxtWebsiteProps = {}) {
    super(scope, id)
    ...
    const certificate =
      props.certificate ?? this.createCertificate(props.hostedZone, props.domainNames)
    ...
    if (!props.disableUpload) {
      /** 
       * The bucket deploy is done before the distribution is created to make sure
       * the files are available when the distribution is created.
       * 
       * This is important because the distribution has a lambda attached that might
       * return html files pointing to assets on the bucket.
       */
      new aws_s3_deployment.BucketDeployment(this, 'PreDeployment', {
    ...
    let responseHeadersPolicy: typeof props.securityHeadersBehavior = undefined;

    if (props.responseHeadersPolicyId?.length) {
      responseHeadersPolicy = ResponseHeadersPolicy.fromResponseHeadersPolicyId(
        this,
        'ReusableResponseHeadersPolicy',
        props.responseHeadersPolicyId,
      );
    } else if (props.securityHeadersBehavior && 'responseHeadersPolicyId' in props.securityHeadersBehavior) {
      responseHeadersPolicy = props.securityHeadersBehavior;
    } else {
    ...
    this.distribution = new aws_cloudfront.Distribution(this, 'Distribution', {
      ...
      minimumProtocolVersion:
        props.minimumProtocolVersion ?? aws_cloudfront.SecurityPolicyProtocol.TLS_V1_2_2021,
    ...
    if (!props.disableUpload) {
       ...
       * return html files pointing to assets on the bucket.
       * 
       * We do it here a second time to take advantage of the distribution
       * invalidation and make sure the files are available on the distribution.
       */
    ...
    if (props.hostedZone) {
      const hostedZones = [props.hostedZone, ...props.alternateHostedZones ?? []]

      props.domainNames?.forEach((domainName) => {
        const hash = crypto.createHash('md5').update(domainName).digest('hex').substring(0, 6)

        const domainHostedZone = hostedZones.find((hz) => domainName.endsWith(hz.zoneName))

        if (!domainHostedZone) {
          throw new Error(
            `The domain name ${domainName} does not belong to any of the provided hosted zones.`,
          )
        }
